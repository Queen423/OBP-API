...
trait Connector extends MdcLoggable {
  ...
  def getCounterpartyByIbanAndAccountId(iban: String, accountId: AccountId, callContext: Option[CallContext]) : OBPReturnType[Box[CounterpartyTrait]] = Future {(Failure(setUnimplementedError), callContext)}
